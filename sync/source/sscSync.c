...
OV_BOOL sync_sscSync_checkResult() {
	return 1;
}
...
OV_BOOL object_isDescendant(const OV_INSTPTR_ov_object pparent,
	...
	if(pparent == pchild || pparent == Ov_PtrUpCast(ov_object, &pdb->root))
		return 1;
	...
	while (pcurrobj != Ov_PtrUpCast(ov_object, &pdb->root)) {
		if(pcurrobj->v_pouterobject) {
			...
			pcurrobj = Ov_PtrUpCast(ov_object,
				Ov_GetParent(ov_containment, pcurrobj));
		}
		if(pcurrobj == pparent) {
			return 1;
		}
...
OV_DLLFNCEXPORT OV_RESULT sync_sscSync_constructor(OV_INSTPTR_ov_object pobj) {
	...
	/* do what */
//	pinst->p_ks.p_apiSet.v_Callback.callbackFunction = &syncCallback;
//	pinst->p_ks.p_apiSet.v_Callback.instanceCalled = Ov_StaticPtrCast(
//		ov_domain, pobj);
...
//	OV_INSTPTR_ksapi_setVar pSetVar = &pinst->p_ks;
//	OV_RESULT result = OV_ERR_OK;
//	OV_INSTPTR_sync_syncVar syncVar = NULL;
//	OV_INSTPTR_ksapi_Variable ksvar = NULL;
//	OV_STRING serverPath = pSetVar->v_path;
//
//	result = Ov_CreateObject(sync_syncVar, syncVar, pSetVar, pobj->v_identifier); //todo: samename?
//	if(result) {
...
//	ksvar = &syncVar->p_var;
//	/* connecting */
//	Ov_Link(sync_syncsyncVar, pobj, syncVar);
//	Ov_Link(ksapi_operationToVariable, &pinst->p_ks, ksvar);
//	/* setting once */
////		OV_ANY tmp = { 0, 0, 0 }
...
//	OV_STRING varPath = ov_path_getcanonicalpath(ksvar, 2);
//	/* adding to states */
//
//	/* setting var */
//	ov_string_print(&syncVar->v_fromPath, "%s.activeStep", varPath);
...
//	ov_string_print(&ksvar->v_path, "%s.initStateName", targetPath);
}
...
void sync_sscSync_swap(OV_INSTPTR_sync_sscSync pinst) {
	;
}
...
//	OV_INSTPTR_ksapi_setVar pSetVar = &pinst->p_ks;
	...
	/* getting values */
//	OV_INSTPTR_ksapi_Variable pvar = NULL;
//	OV_INSTPTR_sync_syncVar syncVar = NULL;
//	OV_INSTPTR_ssc_SequentialStateChart pssc = NULL;
//
//	Ov_ForEachChildEx(ksapi_operationToVariable, pSetVar, pvar, ksapi_Variable)
//	{
//		syncVar = pvar->v_pouterobject;
//		pssc = Ov_GetParent(sync_syncsyncVar, syncVar);
//		pvar->v_varValue.value.vartype = OV_VT_STRING;
//		OV_STRING tmp = ssc_SequentialStateChart_activeStep_get(pssc);
//		ov_string_setvalue(&pvar->v_varValue.value.valueunion.val_string,
//			ssc_SequentialStateChart_activeStep_get(pssc));
//	}
//	pinst->v_round++;
//
//	/* sending */
...
//	pVtblSetVar->m_submit(Ov_StaticPtrCast(ksapi_KSApiCommon, pSetVar));
//
	return result;
}

#define SYNC_STATE_INIT 0
#define SYNC_STATE_FIRST 1
#define SYNC_STATE_SECOND 2
...
OV_RESULT sync_sscSync_execute(OV_INSTPTR_sync_sscSync pinst) {
	...
	if(pinst == 0) {
		/*1.*/
		OV_INSTPTR_fbcomlib_setVar pSetVar = &pinst->p_syncVars;
		...
		result = ks_splitOneStringPath(pinst->v_targetKS, &serverHost, &serverPort,
			&serverName,
			NULL, NULL);
		ov_string_setvalue(&pinst->p_syncVars.v_host, serverHost);
		ov_string_setvalue(&pinst->p_syncVars.v_server, serverName);

		sync_sscSync_createConnections(pinst);
	} else {
		/* check if successful */
		switch (sync_sscSync_checkResult()) {
			// change status
			case OV_ERR_OK:
				// then change;
				return 0;
...
//	if(pinst->v_round <= pinst->v_tryTimes) {
//		/* sending var */
//		sync_sscSync_send(pinst);
//	} else if(pinst->v_round <= 2 * pinst->v_tryTimes) {
//		sync_sscSync_send(pinst);
////		sync_sscSync_calculateProctime();
////		sync_sscSync_setProctime(pinst, );
//		return 0;
//	} else if(pinst->v_round > 2 * pinst->v_tryTimes) {
//
//	}
//	switch (pSetVar->v_status) {
//		case KSAPI_COMMON_INTERNALERROR:
//		case KSAPI_COMMON_EXTERNALERROR:
//		case KSAPI_COMMON_INITIAL:
//			return 1;
//		case KSAPI_COMMON_WAITINGFORANSWER:
//			return 0;
//		case KSAPI_COMMON_REQUESTCOMPLETED:
////					pClient = Ov_SearchChildEx(ov_containment, pSetVar, ksxdr_xdrClient);
//			sync_sscSync_swap(pinst);
//			sync_sscSync_send(pinst);
}
...
//	OV_UINT restssc;
//	if(restssc)
//		sync_sscSync_execute(&(thisGV->p_ks));
//	return;
}

OV_DLLFNCEXPORT void sync_sscSync_typemethod(OV_INSTPTR_fb_functionblock pfb,
		OV_TIME *pltc) {
	/*
	 *   local variables
	 */
