...
OV_DLLFNCEXPORT void ksmsg_executer_typemethod(OV_INSTPTR_ksbase_ComTask this) {
  ...
  if(!pMsg)
    return;
  ov_logfile_info(
      "postExec: %s:%s:%s: %s", pMsg->v_msgBody, pMsg->v_pathComponent.value[0],
      pMsg->v_pathComponent.value[1], pMsg->v_pathComponent.value[2]);
  OV_STRING     srv = NULL;
  OV_STRING     op = NULL;
  ...
  ov_string_setvalue(&pMsgRes->v_refMsgID, pMsg->v_msgID);
  /* beginning */
  if(ov_string_compare(op, "GET") == OV_STRCMP_EQUAL) {
    ...
    for(OV_UINT i = 0; i < len; ++i) {
      params.items_val[i].path_and_name = NULL;
    ...
    msgBody = acplt_simpleMsg_GenerateFlatBody(
        NULL, "RES:SET", 0, NULL, &resIds, &resVals, &resUnits, &resTypes);
    result = PostSys_Message_msgBody_set(pMsgRes, msgBody);
    if(Ov_Fail(result)) {
      Ov_DeleteObject(pMsgRes);
      ov_memstack_unlock();
