...
//  OV_UINT_VEC libPositions = {0, NULL};
  ...
  case INITIAL:
    ...
    result = pVtblClient->m_requestGetVar(pClient, NULL, FORMATLEN, targetpaths,
  ...
  case INFO_RECEIVED:
    ...
    Ov_SetDynamicVectorLength(&libsToSend, pinst->v_libsToSend.veclen, STRING);
    OV_UINT j = 0;
    for (OV_UINT i = 0; i < pinst->v_libsToSend.veclen; i++) {
      if (strvec_contains(&pinst->v_targetLibs, pinst->v_libsToSend.value[i])) {
        ...
        ov_string_setvalue(&libsToSend.value[j], pinst->v_libsToSend.value[i]);
        j++;
      }
    }
    /*
