...
enum state {
	INITIAL, SENT_WAITING, DONE, INTERNAL_ERROR, EXTERNAL_ERROR, STATESLEN
};
...
OV_DLLFNCEXPORT void sendFiles_callback(const OV_INSTPTR_ov_domain this,
	...
	if(pinst->v_postCallback.callbackFunction)
		(*pinst->v_postCallback.callbackFunction)(
			pinst->v_postCallback.instanceCalled, Ov_StaticPtrCast(ov_domain, pinst));
	return;
}
...
OV_RESULT CTree_SendFiles_execute(OV_INSTPTR_CTree_SendFiles pinst) {
	...
	filePositions.value = Ov_HeapMalloc(
		pinst->v_filesToSend.veclen * sizeof(OV_UINT));
	 ...
	 */
	enum {
		FILENAMES, FILEPOSITIONS, BFILES, TRIGGER, SENDFORMATLEN
	};
  ...
  OV_SETVAR_ITEM libSend[SENDFORMATLEN] = {
	...
	result = pVtblClient->m_requestSetVar(pClient, NULL, SENDFORMATLEN, libSend,
		(OV_INSTPTR_ov_domain) pinst, &sendFiles_callback);
