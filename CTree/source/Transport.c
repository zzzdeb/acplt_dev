...
enum State {
	INITIAL, LIBSSENT_WAITING, TREESENT_WAITING, DONE
};
...
OV_RESULT CTree_helper_getClientPointers(OV_INSTPTR_CTree_Transport pCommon,
	...
	*pVtblClient = NULL;
	...
	if(!(*pClient)) { /*	no client found --> create one	*/
		...
		else if(!(*tmpStr)) return OV_ERR_OK;
...
void CTree_Transport_loadlibs_callback(OV_INSTPTR_ov_domain this,
	...
	pVtblClient->m_requestSetVar(pClient, NULL, numberOfItems, items,
		(OV_INSTPTR_ov_domain) pinst, &CTree_Transport_treeupload_callback);
...
OV_DLLFNCEXPORT OV_RESULT CTree_Transport_execute(
		OV_INSTPTR_CTree_Transport pinst) {
	OV_RESULT result = OV_ERR_OK;
	...
	if(ov_string_compare(pinst->v_targetKS, "~") == OV_STRCMP_EQUAL) {
		OV_INSTPTR_CTree_Download pdownload = Ov_StaticPtrCast(CTree_Download,
	...
	result = CTree_LoadLibs_execute_withCallback(&pinst->p_loadlibs,
		Ov_StaticPtrCast(ov_domain, pinst), CTree_Transport_loadlibs_callback);
	if(Ov_OK(result)) {
