...
/**
 * This is a function to create new objects, which have a default name, followed
 * by '_x', where 'x' stands for an int starting with 0. Works just like
 * Ov_CreateObject, except for it needs an OV_RESULT as an additional parameter.
 */
...
OV_DLLFNCEXPORT OV_RESULT PostSys_createAnonymousObject(
  ...
  ov_memstack_lock();
  tmpName = ov_memstack_alloc(
      nameLength +
      12); /*	nameLength plus max langth of uint plus '_' and '\0'	*/
  if(!tmpName) {
    ov_memstack_unlock();
  ...
  do {
    ...
    if(Ov_OK(result))
      break; /*	finished, leave the loop	*/
            ...
            OV_ERR_ALREADYEXISTS) { /*	ALREADYEXISTS is ok so increment i and
                                       continue. Otherwise there is an error and
                                       we leave the loop (while(0) will end)
                                     */
      i++;
    }
