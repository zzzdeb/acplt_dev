...
OV_DLLFNCEXPORT void PostSys_msgHandler_startup(OV_INSTPTR_ov_object pobj) {
	 ...
	 *   local variables
	 */
//    OV_INSTPTR_PostSys_msgHandler pinst = Ov_StaticPtrCast(PostSys_msgHandler, pobj);
...
OV_DLLFNCEXPORT OV_RESULT PostSys_msgHandler_HandleRequest(
	...
	OV_ELEMENT VarElem;
	OV_INSTPTR_PostSys_msgHandler thisMsgHandler = Ov_StaticPtrCast(
		PostSys_msgHandler, this);
	 ...
	 ***********************************************************************************************************************************************************************************************/
	currDataLength = dataReceived->length
			- (dataReceived->readPT - dataReceived->data);
	if(currDataLength < 19) {
		KS_logfile_debug(
			("%s: HandleRequest: Buffer does NOT hold the complete request. waiting some time...", this->v_identifier));
	...
	if(!tempData) {
		ov_memstack_unlock();
		KS_logfile_info(
			("%s: HandleRequest: Message not valid: no msg-Tag. Deleting Data.", this->v_identifier));
		return OV_ERR_BADVALUE;
	}
	if(Ov_Fail(
		acplt_simpleMsg_xml_getAttributeData(msgString, "msg", "hdrL",
			&tempData))) {
		ov_memstack_unlock();
		KS_logfile_info(
			("%s: HandleRequest: Message not valid: no hdrL Attribute. Deleting Data.", this->v_identifier));
		return OV_ERR_BADVALUE;
	}
	...
	if(!hdrLength) {
		ov_memstack_unlock();
		KS_logfile_info(
			("%s: HandleRequest: Message not valid: hdrL 0 or not valid. Deleting Data.", this->v_identifier));
		return OV_ERR_BADVALUE;
	}
	if(Ov_Fail(
		acplt_simpleMsg_xml_getAttributeData(msgString, "msg", "bdyL",
			&tempData))) {
		ov_memstack_unlock();
		KS_logfile_info(
			("%s: HandleRequest: Message not valid: no bdyL Attribute. Deleting Data.", this->v_identifier));
		return OV_ERR_BADVALUE;
	}
	...
	if(!bdyLength) {
		ov_memstack_unlock();
		KS_logfile_info(
			("%s: HandleRequest: Message not valid: bdyL 0 or not valid. Deleting Data.", this->v_identifier));
		return OV_ERR_BADVALUE;
	}
	...
	if(currDataLength < msgLeastLength) { /*	not complete	*/
		...
		if(Ov_Fail(
				acplt_simpleMsg_xml_findElementBegin((char* ) dataReceived->readPT,
						"/msg", &msgEnd)) || !msgEnd) {/*	no </msg>-tag still incomplete	*/
			ov_memstack_unlock();
			KS_logfile_debug(
				("%s: HandleRequest: buffer exceeds size of msgleastLength but no /msg tag found yet", this->v_identifier));KS_logfile_debug(
				("%s: HandleRequest: Buffer does NOT hold the complete request. waiting some time...", this->v_identifier));
			pChannel->v_ConnectionTimeOut = thisMsgHandler->v_timeoutIncomplete;
			return OV_ERR_OK; /*	get called again to process the request next time (if it is complete then).
			 Yes, this could block the ClientHandler for a longer time.	*/
		...
		KS_logfile_debug(
			("%s: HandleRequest: msgEnd is %p; that makes a length of: %u", this->v_identifier, msgEnd, (((OV_STRING) dataReceived->readPT) - msgEnd)));
		...
		if(pChannel) {
			...
			while (VarElem.elemtype != OV_ET_NONE) {
				if(VarElem.elemunion.pvar) {
					KS_logfile_debug(
						("msgHandler %s: iterating over inbox' variables: %s, type: %u", this->v_identifier, VarElem.elemunion.pvar->v_identifier, VarElem.elemunion.pvar->v_vartype & OV_VT_KSMASK));
					if((ov_string_comparei(VarElem.elemunion.pvar->v_identifier,
						"holdConnection") == OV_STRCMP_EQUAL) /*	variable name matches	*/
