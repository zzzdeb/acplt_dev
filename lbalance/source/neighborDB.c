...
OV_DLLFNCEXPORT OV_RESULT lbalance_neighborDB_A_set(
    OV_INSTPTR_lbalance_neighborDB pinst, const OV_BOOL value) {
  OV_RESULT result = OV_ERR_OK;
  if(value && !pinst->v_A) {
    result |= Ov_SetDynamicVectorLength(&pinst->v_loads, 0, UINT);
...
lbalance_neighborDB_typemethod(OV_INSTPTR_fb_functionblock pfb, OV_TIME* pltc) {
  ...
  switch(pinst->v_status) {
    case LB_NDB_INIT:
      ...
      if(Ov_Fail(result)) {
        ov_logfile_error(
            "lbalance_neighborDB: %u: %s: failed to allocate vector", result,
      ...
      Ov_ForEachChildEx(ov_containment, &pinst->p_inbox, pMsg,
        ...
        if(Ov_Fail(result)) {
          ov_logfile_warning("lbalance_neighborDB: parsing msg failed %s  %s",
                             pinst->v_identifier, MsgBody);
        ...
        if(!jsinfo || !cJSON_IsArray(jsinfo)) {
          ov_logfile_warning("lbalance_neighborDB: msg: bad json format at %d",
      ...
      result |=
          Ov_SetDynamicVectorLength(&pinst->v_sysInfos, currentInd, STRING);
      if(Ov_Fail(result)) {
        ov_logfile_error(
